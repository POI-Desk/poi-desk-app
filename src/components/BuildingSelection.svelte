...
<div
	class="absolute md:w-2/5 xl:w-1/5 w-3/5 h-11 bottom-11 left-1/2 z-[100] rounded-full -translate-x-1/2 bg-[#FAF8FC] border-2 border-[#CCC5B9] shadow-[0px_0px_75px_1px_rgba(0,0,0,0.1)] grid grid-cols-{buildings?.length ??
		1} divide-x-2 divide-[#CCC5B9]"
>
	...
	{:else}
		{#each buildings ?? [] as building}
			...
			>
				<p class="select-none">{building.buildingname.split(' ')[1]}</p>
			</button>
		{/each}
	{/if}
</div>
