  ...
  const drawMap = () => {
    ...
    mapData.desks?.map(desk => {
      ...
      deskSvg.$on("click", async () => {
          $selectedDesk = (await getDeskById.fetch({variables: {deskId: desk.pk_deskid}, policy: CachePolicy.NetworkOnly})).data?.getDeskById;
...
<div bind:this={container} class="overflow-hidden h-screen">
  <div
  bind:this={grid}
