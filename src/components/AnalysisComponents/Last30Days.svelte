<script lang="ts">
	...
	import { Last30DaysByLocation } from '$lib/queries/analysisQueries';
...
{#await loadDays()}
	<p>Loading...</p>
{:then data}
	<div class="w-2/5 rounded-3xl flex items-center justify-center">
		<BarChart {data} title="Bookings - {$user.location?.locationname}" />
	</div>
	<div class="flex h-full w-1/2 rounded-3xl items-center justify-center p-2>">
		<div class="flex h-full flex-col gap-2 w-full ">
