<script lang="ts">
	...
	async function loadData() {
		const result = await MonthlyBookings.fetch({
			variables: { year: '2023', month: '12', location: $user.location?.pk_locationid || '' }
		}); //user.location
		...
		{#key comparison.firstMonth || comparison.firstYear}
			{#if comparison.firstYear !== null && comparison.firstMonth !== null}
				{#if comparison.showType === 'Days'}
					<ComparisonBarChart
						title="Total amount of Seats Booked Days"
						year={comparison.firstYear.toString() ?? ''}
						selectedTimePeriod={comparison.firstMonth.toString() ?? ''}
					/>
		...
		{#key comparison.secondMonth || comparison.secondYear}
			{#if comparison.secondYear !== null && comparison.secondMonth !== null}
				{#if comparison.showType === 'Days'}
					<ComparisonBarChart
						title="Total amount of Seats Booked Days"
						year={comparison.secondYear.toString() ?? ''}
						selectedTimePeriod={comparison.secondMonth.toString() ?? ''}
					/>
				{:else if comparison.showType === 'Data'}
					<ComparisonBarChart
						title="Total amount of Seats Booked Data"
						year={comparison.secondYear.toString() ?? ''}
						selectedTimePeriod={comparison.secondMonth.toString() ?? ''}
					/>
				{/if}
...
<div class="card w-48 shadow-xl py-2" data-popup="firstSelectMonth">
	<ListBox rounded="rounded-none">
		{#each selectableTimePeriods as month}
			<ListBoxItem bind:group={comparison.firstMonth} name="medium" value={month}
				>{month}</ListBoxItem
			>
		{/each}
	</ListBox>
...
<div class="card w-48 shadow-xl py-2" data-popup="secondSelectMonth">
	<ListBox rounded="rounded-none">
		{#each selectableTimePeriods as month}
			<ListBoxItem bind:group={comparison.secondMonth} name="medium" value={month}
				>{month}</ListBoxItem
			>
		{/each}
	</ListBox>
